...
class UserManager extends EntityManager
{
	 ...
	 * Update one user
	 */
	public function update($idusers, $firstname, $lastname, $address, $email, $phone, $login, $password){
        $statement = $this->db->prepare("UPDATE users SET firstname = :firstname, lastname = :lastname, address = :address, email = :email, phone = :phone, login = :login, password = :password WHERE idusers = :idusers");
        $statement->execute([
            ':idusers' => $idusers,
            ':firstname' => $firstname,
            ':lastname' => $lastname,
